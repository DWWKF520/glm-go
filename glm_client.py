...
class GLMWebClient:
    def __init__(self, config: AppConfig, logger: Logger) -> None:
        ...
        self._conn_pool: dict[str, http.client.HTTPSConnection] = {}
        self._conn_lock = threading.Lock()

    def _get_connection(self, host: str, port: int) -> http.client.HTTPSConnection:
        key = f"{host}:{port}"
        with self._conn_lock:
            conn = self._conn_pool.get(key)
            if conn is None or conn.sock is None:
                try:
                    conn = http.client.HTTPSConnection(host, port, timeout=self.config.request_timeout)
                    self._conn_pool[key] = conn
                except Exception:
                    return http.client.HTTPSConnection(host, port, timeout=self.config.request_timeout)
            return conn
        ...
        def generate():
            try:
                for event in self._iter_sse_events(response):
                    ...
                    if status in {"finish", "intervene", "tool_call_complete"}:
    ...
    def _open_chat_stream(self, openai_payload: dict[str, object], preferred_account_index: int | None = None):
        requested_model = str(openai_payload.get("model", "glm-4-think"))
        ...
        if refs:
            content_list = list(converted_messages[0]["content"]) # type: ignore
            for item in content_list:
                if isinstance(item, dict) and item.get("type") == "text":
                    cleaned = re.sub(r"\[image:[^\]]+\]", "", str(item.get("text", ""))).strip()
                    item["text"] = cleaned
            converted_messages[0]["content"] = content_list + refs # type: ignore
    ...
    def _iter_sse_events(self, response):
        ...
        def emit_block(block: str):
            ...
            if data_start == -1:
                return None
            payload = block[data_start + 5:].strip()
            if not payload:
                return None
            if debug_enabled:
                debug_dump(self.logger, True, "GLM 原始 SSE block", block)
            if payload == "[DONE]":
                return "[DONE]"
            try:
                parsed = json.loads(payload)
                if debug_enabled:
                    debug_dump(self.logger, True, "GLM 解析后的 SSE payload", parsed)
                return parsed
        ...
        while True:
            ...
            decoded = decoder.decode(raw_chunk, False)
            if "\r\n" in decoded:
                decoded = decoded.replace("\r\n", "\n")
            if decoded:
                pending = pending + decoded if pending else decoded

            sep_pos = pending.find("\n\n")
            if sep_pos != -1:
                while True:
                    block = pending[:sep_pos].strip()
                    pending = pending[sep_pos + 2:]
                    event = emit_block(block)
                    if event == "[DONE]":
                        return
        ...
        if remaining:
            pending = pending + remaining if pending else remaining
            ...
            for item in content:
                ...
                if item_type == "image_url":
                    ...
                    if isinstance(url, str) and url:
                        ref = self._upload_file_reference(url, is_image=True, order=image_order)
                        if ref:
                            refs.append(ref)
                            image_order += 1
                elif item_type == "file":
    ...
    def _upload_file_reference(self, file_url: str, is_image: bool, order: int = 0) -> dict[str, object] | None:
        try:
            ...
            source_id = result.get("file_id") # type: ignore
            if not source_id:
                return None
            if is_image:
                return {
                    ...
                    "image": [
                        {
                            ...
                            "file_id": source_id,
                            "image_url": result.get("file_url", file_url),
                ...
                return {"type": "file", "file": [{"file_id": source_id, "file_url": result.get("file_url", file_url)}]}
        except Exception as exc:
